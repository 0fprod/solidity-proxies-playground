...
contract DelegateCallTest is StdCheats, Test {
    ...
    function test_delegatecall() public {
        target.setFoo(42);
        uint256 fooSlot = stdstore.target(address(target)).sig("foo()").find();
        uint256 barSlot = stdstore.target(address(target)).sig("bar()").find();
    ...
    function test_delegatecall2() public {
        ...
        uint256 fooSlot = stdstore.target(address(caller)).sig("foo()").find();
        uint256 barSlot = stdstore.target(address(caller)).sig("bar()").find();
